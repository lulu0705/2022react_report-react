...
export const getNewposts = async ({ queryKey }) => {
  const [url] = queryKey;
  let data;
  if (url === "")
    data = await axios.get(`${URL}/newpost/all/topthree`);
  return data;
}

export const getProductThree = async ({ queryKey }) => {
  const [url] = queryKey;
  let data;
  if (url === "")
    data = await axios.get(`${URL}/products/all/topthree`);
  return data;
}